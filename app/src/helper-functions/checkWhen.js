...
export const checkWhen = (term, isStdExp, deadline) => {
  ...
  if(isStdExp){
    ...
    const deadlineClassModifier = setClassModifier(when);
    return {
      time: when,
    ...
    if(isAfter){
      ...
      deadlineClassModifier = setClassModifier(when);
    }
    return{
      time: when,
      deadlineClassModifier
...
function setClassModifier(when){
...
// import moment from 'moment';
//
// export const checkWhen = (term, isNotStdExp) => {
//   //----deadline display
//   const currentMonth = moment().format('M');
//   const currentYear = moment().format('YYYY');
//   let currentMonthDeadline = `${currentYear}-${currentMonth}-${term}`;
